...
/**
    ...
    Aurbit Blockchain Technologies
...
contract AURGov {
    ...
    function withdrawTo(address payable _address, uint256 _amt) public {
    ...
    function isAuthed(uint256 _cmdcode) private returns (bool) {
        if (EnableDoubleAuth) {
    ...
    function setPlanetTileBuyFee(address _planet, uint256 _tilebuyfee) public {
        require(isAuthed(11), UNAUTHMSG);
        IPlanet Pl = IPlanet(_planet);
        Pl.setTileBuyFee(_tilebuyfee);
    }

    function setPlanetMiningRate(address _planet, uint256 _miningrate) public {
        require(isAuthed(12), UNAUTHMSG);
        IPlanet Pl = IPlanet(_planet);
        Pl.setMiningRate(_miningrate);
    }

    function setMinAURUnit(address _planet, uint256 _minaurunit) public {
        require(isAuthed(13), UNAUTHMSG);
        IPlanet Pl = IPlanet(_planet);
        Pl.setminAURUnit(_minaurunit);
    }

    function doubleAuth(uint256 _whatwasauthed) private returns (bool) {
